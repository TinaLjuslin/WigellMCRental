...
public class DataInitializer {
    ...
    @Bean
    CommandLineRunner initData(BikeRepository bikeRepository, CustomerService customerService, CustomerRepository customerRepository) {
        return args -> {
            // 1. Skapa cyklar (direkt i repo går bra)
            ...
            // 2. Skapa RIKTIGA kunder via CustomerService
            // Vi kollar om repot är tomt så vi inte spammar Keycloak vid varje omstart
            if (customerRepository.count() == 0) {
                try {
                    customerService.createCustomer(new CustomerCreateDto(
                            "Erik", "Ljuslin", "erik@test.se", "erik_biker", "Lösenord123!",
                            List.of(new AddressCreateDto("Gatan 1", "12345", "Stockholm")), null
                    ));

                    customerService.createCustomer(new CustomerCreateDto(
                            "Anna", "Andersson", "anna@test.se", "anna_biker", "Lösenord123!",
                            List.of(new AddressCreateDto("Vägen 2", "54321", "Göteborg")), null
                    ));

                    customerService.createCustomer(new CustomerCreateDto(
                            "Bettan", "Karlsson", "bettan@test.se", "bettan_b", "Lösenord123!",
                            List.of(new AddressCreateDto("Vägen 3", "54321", "Göteborg")), null
                    ));
